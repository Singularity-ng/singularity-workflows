defmodule QuantumFlow.Executor do
  @moduledoc """
  Database-driven DAG workflow executor matching QuantumFlow's architecture.

  ## Overview

  QuantumFlow.Executor now implements database-driven execution with full DAG support:
  ...
  ## Usage
      ...
      {:ok, result} = QuantumFlow.Executor.execute(MyWorkflow, input, MyApp.Repo)
      ...
      # Error handling
      case QuantumFlow.Executor.execute(MyWorkflow, input, repo) do
  ...
  alias QuantumFlow.DAG.{WorkflowDefinition, RunInitializer, TaskExecutor}
  ...
  ## Examples
      ...
      iex> {:ok, result} = QuantumFlow.Executor.execute(MyWorkflow, %{count: 5}, MyApp.Repo)
      iex> result.count
      6
      ...
      iex> {:ok, result} = QuantumFlow.Executor.execute(
      ...>   DataPipeline,
      ...>   %{},
      ...>   MyApp.Repo,
      ...>   timeout: 60_000
  ...
  def execute(workflow_module, input, repo, opts \\ []) do
    Logger.info("QuantumFlow.Executor: Starting workflow",
    ...
    with {:ok, definition} <- WorkflowDefinition.parse(workflow_module),
      ...
      case result do
        {:ok, output} ->
          Logger.info("QuantumFlow.Executor: Workflow completed",
            workflow: workflow_module,
            run_id: run_id
          )
        ...
        {:error, reason} ->
          Logger.error("QuantumFlow.Executor: Workflow failed",
            workflow: workflow_module,
            run_id: run_id,
      ...
      {:error, reason} ->
        Logger.error("QuantumFlow.Executor: Workflow initialization failed",
          workflow: workflow_module,
          reason: inspect(reason)
        )
  ...
  def execute_dynamic(workflow_slug, input, step_functions, repo, opts \\ [])
    ...
    Logger.info("QuantumFlow.Executor: Starting dynamic workflow",
      workflow_slug: workflow_slug,
      input_keys: Map.keys(input)
    )

    alias QuantumFlow.DAG.DynamicWorkflowLoader
    ...
    with {:ok, definition} <- DynamicWorkflowLoader.load(workflow_slug, step_functions, repo),
      ...
      case result do
        {:ok, output} ->
          Logger.info("QuantumFlow.Executor: Dynamic workflow completed",
            workflow_slug: workflow_slug,
            run_id: run_id
          )
        ...
        {:error, reason} ->
          Logger.error("QuantumFlow.Executor: Dynamic workflow failed",
            workflow_slug: workflow_slug,
            run_id: run_id,
            reason: inspect(reason)
          )
      ...
      {:error, reason} ->
        Logger.error("QuantumFlow.Executor: Dynamic workflow initialization failed",
          workflow_slug: workflow_slug,
          reason: inspect(reason)
        )
  ...
  ## Examples
      ...
      iex> {:ok, status, info} = QuantumFlow.Executor.get_run_status(run_id, MyApp.Repo)
      iex> status
      :in_progress
      iex> info
      ...
      iex> {:ok, :completed, output} = QuantumFlow.Executor.get_run_status(completed_run_id, repo)
      iex> Map.get(output, "result")
      "success"

      # Check failed workflow
      iex> {:ok, :failed, error} = QuantumFlow.Executor.get_run_status(failed_run_id, repo)
      iex> error
      "Step :validate failed: Invalid input"

      # Non-existent run
      iex> QuantumFlow.Executor.get_run_status("00000000-0000-0000-0000-000000000000", repo)
      {:error, :not_found}
  """
  ...
  def get_run_status(run_id, repo) do
    ...
    case repo.get(QuantumFlow.WorkflowRun, run_id) do
      nil ->
        {:error, :not_found}
  ...
  defp calculate_progress(run_id, repo) do
    ...
    total_steps =
      from(s in QuantumFlow.StepState,
