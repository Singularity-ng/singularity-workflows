defmodule QuantumFlow.Orchestrator do
  @moduledoc """
  Hierarchical Task Directed Acyclic Graph support for quantum_flow.
  
  Provides generic HTDAG functionality that can be used by any Elixir application.
  This module bridges goal-driven task decomposition with quantum_flow workflow execution.
  
  ## Features
  
  - **Goal Decomposition**: Convert high-level goals into hierarchical task graphs
  - **Workflow Generation**: Transform HTDAG tasks into quantum_flow workflows
  ...
  ## Usage
      ...
      {:ok, result} = QuantumFlow.Orchestrator.execute_goal(
  ...
  HTDAG integrates with quantum_flow's existing components:
  ...
  - **Disambiguates from**:
    - `QuantumFlow.Executor` (low-level task execution)
    - `QuantumFlow.WorkflowComposer` (high-level API wrapper)
    - `QuantumFlow.FlowBuilder` (raw workflow creation)
  ...
  autonomous_decomposition, task_dependencies, quantum_flow_orchestration, goal_execution,
  ...
  create_workflow(task_graph, step_functions)
      │ (converts to quantum_flow steps)
      ↓
  Workflow Object
  ...
  ## Example
      ...
      {:ok, tasks} = QuantumFlow.Orchestrator.decompose_goal(
  ...
  def decompose_goal(goal, decomposer, opts \\ []) do
    ...
    config = QuantumFlow.Orchestrator.Config.get_execution_config(opts)
    ...
    if QuantumFlow.Orchestrator.Config.feature_enabled?(:notifications) do
      goal_id = generate_goal_id(goal)
      QuantumFlow.OrchestratorNotifications.broadcast_decomposition(goal_id, :started, %{
        goal: goal,
        max_depth: max_depth
    ...
    try do
      case Task.await(Task.async(fn -> decomposer.(goal) end), timeout) do
        {:ok, tasks} when is_list(tasks) ->
          ...
          if QuantumFlow.Orchestrator.Config.feature_enabled?(:notifications) do
            goal_id = generate_goal_id(goal)
            QuantumFlow.OrchestratorNotifications.broadcast_decomposition(goal_id, :completed, %{
        ...
        {:error, reason} ->
          ...
          if QuantumFlow.Orchestrator.Config.feature_enabled?(:notifications) do
            goal_id = generate_goal_id(goal)
            QuantumFlow.OrchestratorNotifications.broadcast_decomposition(goal_id, :failed, %{
  ...
  - `{:ok, workflow}` - quantum_flow workflow that can be executed
  ...
  ## Example
      ...
      {:ok, workflow} = QuantumFlow.Orchestrator.create_workflow(task_graph, step_functions)
  """
  ...
  def create_workflow(task_graph, step_functions, opts \\ []) do
    ...
    try do
      # Convert HTDAG tasks to quantum_flow workflow steps
  ...
  @doc """
  Execute a goal using HTDAG decomposition and quantum_flow execution.
  ...
  def execute_goal(goal, decomposer, step_functions, repo, opts \\ []) do
    with {:ok, task_graph} <- decompose_goal(goal, decomposer, opts),
      ...
      QuantumFlow.Executor.execute(workflow, %{goal: goal}, repo)
    end
  end
  ...
  def get_execution_stats(workflow_name \\ nil, repo, opts \\ []) do
    ...
    workflow_id = if workflow_name do
      case QuantumFlow.Orchestrator.Repository.get_workflows_by_name(workflow_name, repo, limit: 1) do
    ...
    QuantumFlow.Orchestrator.Repository.get_execution_stats(repo, Keyword.put(opts, :workflow_id, workflow_id))
  end
