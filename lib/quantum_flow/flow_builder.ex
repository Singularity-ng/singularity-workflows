defmodule QuantumFlow.FlowBuilder do
  ...
  ### Create a workflow
      ...
      {:ok, workflow} = QuantumFlow.FlowBuilder.create_flow("ai_generated_workflow", repo,
        max_attempts: 5,
        timeout: 60
      )
  ...
  ### Add steps with dependencies
      ...
      {:ok, _} = QuantumFlow.FlowBuilder.add_step("ai_generated_workflow", "fetch_data", [], repo)

      {:ok, _} = QuantumFlow.FlowBuilder.add_step("ai_generated_workflow", "process", ["fetch_data"], repo,
        initial_tasks: 10,  # Map step - process 10 items
        max_attempts: 3
      )

      {:ok, _} = QuantumFlow.FlowBuilder.add_step("ai_generated_workflow", "save", ["process"], repo)
  ...
  def create_flow(workflow_slug, _repo, opts \\ []) do
    ...
    with :ok <- validate_workflow_slug(workflow_slug),
      ...
      QuantumFlow.FlowOperations.create_flow(workflow_slug, max_attempts, timeout)
    else
  ...
  def add_step(workflow_slug, step_slug, depends_on, _repo, opts \\ []) do
    ...
    with :ok <- validate_workflow_slug(workflow_slug),
      ...
      # Use Elixir implementation instead of PostgreSQL function to bypass PG17 parser bug
      QuantumFlow.FlowOperations.add_step(
        workflow_slug,
        step_slug,
        depends_on,
        step_type,
