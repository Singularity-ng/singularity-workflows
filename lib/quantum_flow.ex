defmodule QuantumFlow do
  @moduledoc """
  QuantumFlow - Complete workflow orchestration for Elixir.
  ...
  quantum_flow supports TWO ways to define workflows:
  ...
  Create workflows at runtime via FlowBuilder API:
      ...
      {:ok, _} = QuantumFlow.FlowBuilder.add_step("ai_workflow", "step1", [], repo)

      step_functions = %{step1: fn input -> {:ok, input} end}
      QuantumFlow.Executor.execute_dynamic("ai_workflow", input, step_functions, repo)

  **Both approaches use the same execution engine!**

  ## HTDAG Integration

  QuantumFlow includes `QuantumFlow.HTDAG` for goal-driven workflow creation:
  ...
  QuantumFlow includes `QuantumFlow.Notifications` for real-time message delivery:
      ...
      {:ok, pid} = QuantumFlow.Notifications.listen("chat_messages", MyApp.Repo)

  ## Architecture

  quantum_flow uses the same architecture as QuantumFlow (TypeScript):
  ...
  | Feature | QuantumFlow (TypeScript) | quantum_flow (Elixir) |
  |---------|---------------------|---------------------|
  | DAG Syntax | ✅ | ✅ |
  | pgmq Integration | ✅ | ✅ |
  | Parallel Execution | ✅ | ✅ |
  ...
  **Result: 100% Feature Parity** 🎉
  ...
  See `QuantumFlow.Executor` for execution options and `QuantumFlow.DAG.WorkflowDefinition`
  for workflow syntax details.

  ## Real-time Notifications

  quantum_flow includes `QuantumFlow.Notifications` for real-time workflow events with comprehensive logging:
  ...
  ### Integration Examples
      ...
      {:ok, _} = QuantumFlow.Notifications.send_with_notify("genesis_learning", %{
        type: "rule_evolved",
  ...
  quantum_flow includes `QuantumFlow.Orchestrator` for goal-driven workflow creation:
  ...
  defdelegate send_with_notify(queue, message, repo), to: QuantumFlow.Notifications
  defdelegate listen(queue, repo), to: QuantumFlow.Notifications
  defdelegate unlisten(listener_pid, repo), to: QuantumFlow.Notifications
  defdelegate notify_only(channel, payload, repo), to: QuantumFlow.Notifications

  @doc """
  Returns the current version of quantum_flow.

  ## Examples

      iex> QuantumFlow.version()
      "0.1.0"
  """
