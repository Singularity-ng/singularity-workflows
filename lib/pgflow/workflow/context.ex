defmodule Pgflow.Workflow.Context do
  @moduledoc """
  Lightweight workflow context struct used when invoking Pgflow workflow
  steps outside of the orchestration runtime.

  The production Pgflow executor builds workflow contexts dynamically as maps
...
defmodule PGFlow.Workflow.Context do
  @moduledoc """
  CamelCase alias of `Pgflow.Workflow.Context` provided for backwards
  compatibility with modules that still reference the legacy `PGFlow`
  namespace.
  """

  @doc false
  defdelegate __struct__(), to: Pgflow.Workflow.Context
  @doc false
  defdelegate __struct__(kv), to: Pgflow.Workflow.Context, as: :__struct__
end
