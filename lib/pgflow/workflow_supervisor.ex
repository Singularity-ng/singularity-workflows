defmodule PGFlow.WorkflowSupervisor do
  @moduledoc """
  Supervisor wrapper that keeps backwards compatibility with the original
  `PGFlow.WorkflowSupervisor` API used throughout the umbrella.

  Internally it starts the newer `Pgflow.Workflow` process that orchestrates
  executions.  Multiple supervisors can run concurrently – the shared registry
  used by `Pgflow.Workflow` is started on demand.
  """
  ...
  def init(opts) do
    ...
    children = [
      %{
        id: {Pgflow.Workflow, workflow_module},
        start: {Pgflow.Workflow, :start_link, [workflow_name, workflow_module, [repo: repo]]},
        restart: :permanent,
        shutdown: 15_000,
        type: :worker
      }
  ...
  def start_workflow(workflow_module, opts \\ []) do
    ...
    workflow_name = Keyword.get(opts, :workflow_name, default_workflow_name(workflow_module))
    repo = resolve_repo(opts, workflow_module)
    Pgflow.Workflow.start_link(workflow_name, workflow_module, repo: repo)
  end
  ...
  def list_workflows do
    case Process.whereis(Pgflow.WorkflowRegistry) do
      nil ->
        []

      _pid ->
        Registry.select(Pgflow.WorkflowRegistry, [{{:"$1", :_, :_}, [], [:"$1"]}])
    end
  end
  ...
  defp ensure_registry_started do
    case Process.whereis(Pgflow.WorkflowRegistry) do
      nil ->
        case Registry.start_link(keys: :unique, name: Pgflow.WorkflowRegistry) do
          {:ok, _pid} -> :ok
          {:error, {:already_started, _pid}} -> :ok
          {:error, reason} -> raise "Failed to start Pgflow.WorkflowRegistry: #{inspect(reason)}"
        end
  ...
  defp resolve_repo(opts, workflow_module) do
    cond do
      ...
      true ->
        raise "PGFlow.WorkflowSupervisor could not determine an Ecto repo. Pass :repo option explicitly."
    end
  end
