defmodule PGFlow.Workflow do
  @moduledoc """
  Backwards-compatible API wrapper around the lower-case `Pgflow` workflow
  modules.

  The original NATS-based orchestration code referenced a `PGFlow.Workflow`
  module that exposed helper functions such as `execute/2` and `status/1`.
  Those calls now delegate to the fully-featured `Pgflow` engine while keeping
  ...
  alias Pgflow.DAG.{RunInitializer, TaskExecutor, WorkflowDefinition}
  alias Pgflow.{StepState, WorkflowRun}
  ...
  def execute(workflow_module, input, opts \\ []) when is_map(input) do
    ...
    with {:ok, definition} <- WorkflowDefinition.parse(workflow_module),
      ...
      {:error, reason} ->
        Logger.error("PGFlow.Workflow failed to start run",
  ...
  defp start_async_runner(run_id, definition, repo, opts) do
    ...
    Task.Supervisor.start_child(PGFlow.TaskSupervisor, fn ->
  ...
  defp resolve_repo(opts, workflow_module) do
    cond do
      ...
      true ->
        raise "PGFlow.Workflow could not determine an Ecto repo. Pass :repo option explicitly."
    end
  end
