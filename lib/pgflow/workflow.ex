defmodule Pgflow.Workflow do
  @moduledoc """
  Pgflow.Workflow - Production-grade workflow orchestration module.
  ...
  use GenServer
  require Logger
  alias Pgflow.{Executor, Notifications}
  ...
  def start_link(workflow_name, workflow_module, opts \\ []) do
    ...
    unless function_exported?(workflow_module, :__workflow_steps__, 0) do
      ...
      case GenServer.start_link(__MODULE__, state, name: via_tuple(workflow_name)) do
        {:ok, pid} ->
          Logger.info("Pgflow.Workflow: Started workflow process",
            workflow_name: workflow_name,
            workflow_module: workflow_module,
            pid: inspect(pid)
          )
        ...
        {:error, reason} = error ->
          Logger.error("Pgflow.Workflow: Failed to start workflow process",
            workflow_name: workflow_name,
  ...
  @impl GenServer
  def init(state) do
    Logger.debug("Pgflow.Workflow: Initialized",
  ...
  def handle_call({:enqueue, step_name, input}, _from, state) do
    ...
    Logger.info("Pgflow.Workflow: Enqueuing job",
      workflow_name: state.workflow_name,
      step_name: step_name,
      job_id: job_id
    )
    ...
    case Executor.execute(state.workflow_module, workflow_input, state.repo, timeout: 300_000) do
      ...
      {:error, reason} ->
        Logger.error("Pgflow.Workflow: Job execution failed",
          workflow_name: state.workflow_name,
          job_id: job_id,
          step_name: step_name,
          reason: inspect(reason)
  ...
  def handle_call({:update, action, job_id, job_data}, _from, state) do
    Logger.debug("Pgflow.Workflow: Processing update",
      workflow_name: state.workflow_name,
      action: action,
      job_id: job_id
    )
  ...
  defp via_tuple(workflow_name) do
    {:via, Registry, {Pgflow.WorkflowRegistry, workflow_name}}
  end
