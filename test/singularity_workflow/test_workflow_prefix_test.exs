defmodule Singularity.Workflow.TestWorkflowPrefixTest do
  use ExUnit.Case, async: true

  alias Singularity.Workflow.TestWorkflowPrefix
  ...
  describe "start/0" do
    test "returns a string with the expected prefix format" do
      ...
      assert String.starts_with?(prefix, "singularity_workflow_test_")
      assert String.ends_with?(prefix, "_")
      # Should be 8 characters for the UUID slice + prefix + suffix
      # "singularity_workflow_test_" + 8 chars + "_"
  ...
  describe "module structure" do
    test "has comprehensive module documentation" do
      {:docs_v1, _, _, _, mod_docs, _, _} = Code.fetch_docs(Singularity.Workflow.TestWorkflowPrefix)
    ...
    test "defines expected function specs" do
      ...
      functions = Singularity.Workflow.TestWorkflowPrefix.__info__(:functions)
