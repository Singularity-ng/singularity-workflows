defmodule Singularity.Workflow.StartTasksDoubleClaimTest do
  @moduledoc """
  Falsifier: second start_tasks on the same msg_id must return 0 rows and
  must not steal or bump attempts after the first worker claimed.
  """
  ...
  test "second worker cannot claim already-started message" do
    slug = "test_dbl_claim_#{System.unique_integer([:positive])}"
    ...
    {:ok, %{rows: first}} =
      Repo.query("SELECT * FROM start_tasks($1, ARRAY[$2]::bigint[], $3)", [slug, msg_id, w1])

    {:ok, %{rows: second}} =
      Repo.query("SELECT * FROM start_tasks($1, ARRAY[$2]::bigint[], $3)", [slug, msg_id, w2])

    assert length(first) == 1
    assert second == []

    {:ok, %{rows: [[status, claimed_by, attempts]]}} =
      Repo.query(
        """
        SELECT status, claimed_by, attempts_count
