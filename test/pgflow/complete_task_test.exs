  ...
  test "complete_task marks task completed and updates dependent state" do
    case Pgflow.SqlCase.connect_or_skip() do
      ...
      conn ->
        ...
        # Insert step metadata and state
        Postgrex.query!(
          conn,
          "INSERT INTO workflow_steps (workflow_slug, step_slug, step_type) VALUES ($1, $2, 'single')",
          [workflow_slug, "parent"]
        )

        Postgrex.query!(
          conn,
          "INSERT INTO workflow_steps (workflow_slug, step_slug, step_type) VALUES ($1, $2, 'map')",
          [workflow_slug, "child"]
        )

        Postgrex.query!(
          conn,
          "INSERT INTO workflow_step_states (run_id, step_slug, status, remaining_tasks, remaining_deps, initial_tasks) VALUES ($1, $2, 'created', 1, 0, NULL)",
          [id, "parent"]
        )

        Postgrex.query!(
          conn,
          "INSERT INTO workflow_step_states (run_id, step_slug, status, remaining_tasks, remaining_deps, initial_tasks) VALUES ($1, $2, 'created', 0, 1, NULL)",
          [id, "child"]
        )

        Postgrex.query!(
          conn,
          "INSERT INTO workflow_step_tasks (run_id, step_slug, task_index, status) VALUES ($1, $2, 0, 'started')",
          [id, "parent"]
        )
        ...
        Postgrex.query!(
          conn,
          "INSERT INTO workflow_step_dependencies (run_id, step_slug, depends_on_step) VALUES ($1, $2, $3)",
          [id, "child", "parent"]
        )

        # Call complete_task with an array output so child initial_tasks will be set
        Postgrex.query!(conn, "SELECT complete_task($1::uuid, $2::text, $3::int, $4::jsonb)", [
          id,
          "parent",
          0,
          Jason.encode!([1, 2, 3])
