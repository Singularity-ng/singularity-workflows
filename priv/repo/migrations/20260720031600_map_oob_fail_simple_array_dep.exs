defmodule Singularity.Workflow.Repo.Migrations.MapOobFailSimpleArrayDepV2 do
  @moduledoc """
  Simplify map OOB detection to completed array-typed dep task outputs.
  """
  use Ecto.Migration
...
LOOP
  ...
  IF NOT EXISTS (
    ...
    SET status = 'failed', failed_at = NOW(),
        error_message = '[MAP_OOB] task_index exceeds parent array length'
    WHERE run_id = r.run_id AND step_slug = r.step_slug AND status <> 'failed';
...
$$
    ...
    'Claims queued tasks. Map OOB (array dep length <= task_index) fails+archives; defers step/run fail while in-bounds remain.';
    """)
  end
