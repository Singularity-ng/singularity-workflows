defmodule Singularity.Workflow.Repo.Migrations.AggregateMapDepOutputsInStartTasks do
  @moduledoc """
  Aggregate map dependency outputs into an ordered JSON array in start_tasks.

  Purpose: reduce/save steps after HTDAG map fan-out must see all task outputs
  (by task_index), not one arbitrary completed row from a multi-row JOIN.

  Consumer: public.start_tasks → TaskExecutor / C21. Map child slicing
  (array[task_index]) unchanged. Non-map single-task deps unchanged.
  """
  use Ecto.Migration
  ...
  def up do
    ...
    'Claims queued tasks. Map child: input=parent array[task_index]. Map dep into non-map: nested dep key is jsonb array of outputs by task_index. Else: run.input || flat object merge || nested deps.';
    """)
  end
