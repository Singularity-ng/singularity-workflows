defmodule Singularity.Workflow.Repo.Migrations.MapOobDeferRunFail do
  @moduledoc """
  Map OOB: fail excess indexes at claim without immediately failing the run,
  so in-bounds map workers in the same claim batch still execute.
  """
  use Ecto.Migration
...
BEGIN
  ...
  -- Fail map claims past parent array length (no nested-fallback execute).
  FOR r IN
    SELECT
      st.run_id AS run_id,
...
$$
    ...
    'Claims queued tasks. Map OOB fails+archives excess indexes; defers step/run fail until no queued/started remain.';
    """)
  end
