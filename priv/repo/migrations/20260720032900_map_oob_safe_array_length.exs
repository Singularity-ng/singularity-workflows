defmodule Singularity.Workflow.Repo.Migrations.MapOobSafeArrayLength do
  @moduledoc """
  Guard jsonb_array_length with typeof CASE (PG AND may not short-circuit).
  """
  use Ecto.Migration
...
BEGIN
  ...
  -- Fail map claims past parent array length (single array-typed dep output).
FOR r IN
  SELECT
    st.run_id AS run_id,
    st.step_slug AS step_slug,
    st.task_index AS task_index,
    st.message_id AS message_id
  FROM workflow_step_tasks st
  JOIN workflow_steps ws
    ON ws.workflow_slug = p_workflow_slug
    AND ws.step_slug = st.step_slug
  WHERE st.workflow_slug = p_workflow_slug
    ...
    AND ws.step_type = 'map'
    AND EXISTS (
      SELECT 1
      FROM workflow_step_dependencies wsd
      JOIN workflow_step_tasks dep
        ON dep.run_id = st.run_id
        AND dep.step_slug = wsd.depends_on_step
        AND dep.status = 'completed'
        AND dep.output IS NOT NULL
        AND CASE
              WHEN jsonb_typeof(dep.output) = 'array'
              THEN jsonb_array_length(dep.output) <= st.task_index
              ELSE false
            END
      WHERE wsd.run_id = st.run_id
        AND wsd.step_slug = st.step_slug
    )
LOOP
  ...
  SET
    status = 'failed',
    failed_at = NOW(),
    error_message = '[MAP_OOB] task_index exceeds parent array length',
    claimed_by = NULL,
    claimed_at = NULL
  WHERE workflow_step_tasks.run_id = r.run_id
    AND workflow_step_tasks.step_slug = r.step_slug
    AND workflow_step_tasks.task_index = r.task_index
    AND workflow_step_tasks.status = 'started';

  UPDATE workflow_step_states ss
  SET remaining_tasks = GREATEST(ss.remaining_tasks - 1, 0)
  WHERE ss.run_id = r.run_id
    AND ss.step_slug = r.step_slug;

  IF r.message_id IS NOT NULL THEN
    PERFORM pgmq.archive(p_workflow_slug, ARRAY[r.message_id]);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM workflow_step_tasks t
    WHERE t.run_id = r.run_id AND t.step_slug = r.step_slug
      AND t.status IN ('queued', 'started')
  ) THEN
    UPDATE workflow_step_states
    SET status = 'failed', failed_at = NOW(),
        error_message = '[MAP_OOB] task_index exceeds parent array length'
    WHERE workflow_step_states.run_id = r.run_id AND workflow_step_states.step_slug = r.step_slug AND workflow_step_states.status <> 'failed';

    UPDATE workflow_runs
    SET status = 'failed', failed_at = NOW(),
        error_message = '[MAP_OOB] task_index exceeds parent array length'
    WHERE id = r.run_id AND status <> 'failed';
  END IF;
END LOOP;
      ...
      THEN COALESCE(
        (
          ...
          WHERE d.d_run_id = c.t_run_id
            ...
            AND CASE WHEN jsonb_typeof(d.d_output) = 'array' THEN jsonb_array_length(d.d_output) > c.t_task_index ELSE false END
...
$$
    ...
    'Claims queued tasks. Map OOB uses safe array-length checks; map agg + slice preserved.';
    """)
  end
